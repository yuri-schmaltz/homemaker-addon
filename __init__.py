...
"""Top-level package for Homemaker Blender add-on."""

# Blender-specific dependencies are optional. When running outside Blender
# (for example during automated testing or documentation builds) these
# imports may fail. We handle that gracefully so the package can still be
# imported in a limited form.
...
except ImportError:  # pragma: no cover - executed when Blender deps missing
    IfcStore = None  # type: ignore[assignment]
...
if _BLENDER_AVAILABLE:
    ...
    def register():
        """Placeholder register function when Blender API is missing."""
        return None

    def unregister():
        """Placeholder unregister function when Blender API is missing."""
        return None
